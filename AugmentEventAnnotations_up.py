...
def importPSIevents(PSIpath,species):
    ...
    for line in open(PSIpath,'rU').xreadlines():
        ...
        if header:
            ...
            header=False
            continue
        primary_junction = values[5]
        secondary_junction = values[6]
...
def updatePSIAnnotations(PSIpath, species, psievents, terminal_exons, junctionPairFeatures):
    ...
    for line in open(PSIpath,'rU').xreadlines():
        ...
        psiJunction_primary = psiJunction[5]
        psiJunction_secondary = psiJunction[6]
        ...
        try:
            if event==None:
                primary_exons=string.split(psiJunction[5],":")
                secondary_exons=string.split(psiJunction[6],":")
