...
import ExpandClusters_v2_test_KD as ExpandSampleClusters
...
def FindTopUniqueEvents(Guidefile,psi,Guidedir):
    ...
    for line in open(Guidefile,'rU').xreadlines():
        if head==0:
            ...
            if abs(float(q[9]))>0.2 and float(q[10])<0.01:
                try:
...
def CompleteWorkflow(InputFile,EventAnnot,turn):
        ...
        if Rank>1:
           ...
           # NMFResult,BinarizedOutput,Metadata,Annotation=NMF_Analysis.NMFAnalysis(NMFinput,Rank,turn)
            ...
            #Guidedir="/Volumes/Pass/Knockdown_Classification/RBP-KD/Misc-GEO-experiments/Events-dPSI_0.1_rawp_lymphoblast"
            #PSIdir="/Volumes/Pass/Knockdown_Classification/RBP-KD/Misc-GEO-experiments/ExpressionProfiles_Lymphoblast"
            #BinarizedOutput="/Volumes/Pass/Knockdown_Classification/RBP-KD/Misc-GEO-experiments/BInarized_Lymphoblast.txt"
            ...
            for filename in os.listdir(Guidedir):
                if filename.startswith("PSI."):
                    ...
                    if omitcluster==0:
                        ...
                        name.append(psi)
                        counter+=1
            InputFile2="/Volumes/Pass/Knockdown_Classification/RBP-KD/ENCODE/Hs_RNASeq_top_alt_junctions-PSI-clust-filtered.txt"
            #InputFile2="/Volumes/Pass/Knockdown_Classification/RBP-KD/Misc-GEO-experiments/exp.KD_splicing.txt"
            #upd_guides=[]
            #head=0
            #for line in open(InputFile2,'rU').xreadlines():
            #    if head==0:
            #        head=1
            #        continue
            #    else:
