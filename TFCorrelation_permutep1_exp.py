...
def worker(i):
 ...
 for l in range(len(lines)):
    ...
    for qw in tfs:
     if t1 == qw:
      export1=open(t1+str(i)+"_permute1_exp.txt","a")
      t=t[1:]
      for i in t:
         t2.append(int(i))
      ...
      for iter in range(100):
         ...
         for k in range(len(lines)):
               ...
               #     continue
               for i in range(len(t)-1):
                list1.append(float(newarray[i]))
                list2.append(float(p[i]))
                #if i in newones:
...
if __name__ == '__main__':
    ...
    tfs=genelist(query_tf_file)
    gene_labels=genelist(query_exp_file)
    processes = [multiprocessing.Process(target=worker, args=(x,)) for x in range(10)]
    for p in processes:
	p.start()
    for p in processes:
	p.join()
