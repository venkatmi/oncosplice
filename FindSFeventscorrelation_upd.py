...
def findcorrelations():
    # input psi file  
    SplicingeventsFile="/Volumes/Pass/GeneSplicingCOrrelations/round2_input-filtered.txt"
    # filtered expression file for selected genes
    SFexp_file = open("/Volumes/Pass/GeneSplicingCOrrelations/Tophat_KAT6B.txt", "rU")
    lines = SFexp_file.readlines()
    SFexp_file.close()
    
    #writing the splicing factor results
    export=open('Tophat_KAT6B_SF_comps_round2.txt','w')
    ...
    for exp in lines:
       ...
       # export=open('/Users/meenakshi/Documents/ICGS_guidefiles/Guide3_firstroundicgs-correlations.txt','w')
        ...
        filename=SplicingeventsFile[:-4]+"_"+sfname+"_"+str(count)+"Tophat_round2_KAT6B.txt"
        if count>20:
