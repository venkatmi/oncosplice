...
def FindTopUniqueEvents(Guidefile,psi,Guidedir):
    head=0
    guidekeys=[]
    exportnam=os.path.join(Guidedir,"Splicingeventcount_round1.txt")
    export_class=open(exportnam,"a")
    exportnam1=os.path.join(Guidedir,"SplicingEvents_round1.txt")
    export_class2=open(exportnam1,"a")
    #commonkeys=[]
    tempkeys={}
    global upd_guides
    global train
    ...
    try:
        if len(unique_clusters[0])>1:     
            ...
            if len(unique_clusters[0])>150:
                ...
                guidekeys=unique_clusters[0][0:50]
                #print guidekeys
                #sys.exit()
                for i in range(0,len(guidekeys)):
                    export_class2.write(Guidefile+"\t"+guidekeys[i][0]+"\n")
...
def CompleteWorkflow(InputFile,EventAnnot,turn):
        ...
        #Guidefile="/Volumes/Pass/Leucegene_Complete/ICGS/Clustering-exp.splicing-filteredcor_depleted-Guide3 GMDS ENSG00000112699 E14.1-E17.1 ENSG000001-hierarchical_euclidean_correlation.txt"
        ...
        #    Guidedir="/Volumes/Pass/Leucegene_Complete/round1/round2_sorted/Events-dPSI_0.1_adjp"
        #    PSIdir="/Volumes/Pass/Leucegene_Complete/round1/round2_sorted/ExpressionProfiles"
        #    BinarizedOutput="/Volumes/Pass/Leucegene_Complete/ExpressionInput/round2_sorted/exp.splicing-filteredcor_depleted-filteredNMFsnmf_binary30.txt"
        #    global upd_guides
        #    upd_guides=[]
        #    name=[]
        #    group=[]
        ...
        #    Finalclusters,FinalCentroids=ExpandSampleClusters.Classify(header,train,output_file,grplst,name)
            NMFResult="/Volumes/Pass/Target_SplicingData/exp.splicing_filter-filtered-filteredNMFsnmf_versionr2.txt"
