...
def FindTopUniqueEvents(Guidefile,psi,Guidedir):
    head=0
    guidekeys=[]
    #exportnam=os.path.join(Guidedir,"SplicingeventCount.txt")
...
def Classify(header,Xobs1,train2,output_file,grplst,name):
    ...
    exportnam=output_file[:-4]+'SVC_one.txt'
    export_class_svc=open(exportnam,"a")
    export_class_svc.write(header)
    exportnam1=output_file[:-4]+'SVC_one_decision.txt'
    export_class1=open(exportnam1,"a")
    for iqt in range(0,len(header1)-1):
        export_class1.write("\t"+header1[iqt+1])
    export_class1.write("\n")
    exportnam=output_file[:-4]+'Random_one.txt'
    export_class_rf=open(exportnam,"a")
    export_class_rf.write(header)
    Y=zip(*Y)
    Y=np.array(Y)
    print Y.shape
    ...
    #Xobs=np.array(Xobs)
        ...
        q=regr.predict(Y)
        count=1
        export_class_knn.write(name[iq])
        for i in q:
            export_class_knn.write("\t"+str(i))
            count+=1
        export_class_knn.write("\n")
        ...
        #np.savetxt("/Volumes/MyPassport/Users/saljh8/Desktop/dataAnalysis/SalomonisLab/Leucegene/July-2017/PSI/ExpressionProfiles/DataPlots/complete_SVC.txt",q)
        ...
        q=regr.predict(Y)
        count=1
        export_class_rf.write(name[iq])
        for i in q:
            export_class_rf.write(header[count]+"\t"+str(i)+"\n")
            count+=1
        export_class_rf.write("\n")
    ...
    for exp1 in open(NMF_annot,"rU").xreadlines():
        ...
        if head==0:
            ...
            for i in range(1,len(lin)):
                if lin[i]=='1':
                    mapping[1].append(header[i])
                else:
                    mapping[0].append(header[i])
                head2=0
            #print len(mapping[1])
...
#keylabel="/Users/meenakshi/Documents/leucegene/ExpressionInput/exp.round2_glmfilteredKmeans_label.txt"
    ...
    train,train2=TrainDataGeneration(output_file,NMF_annot,name)
    grplst.append(group)
    print grplst
    Classify(header,train,train2,output_file,grplst,name)